...
def calculate_roc(thresholds, embeddings1, embeddings2, actual_issame, nrof_folds=10):
    ...
    k_fold = KFold(n=nrof_pairs, n_folds=nrof_folds, shuffle=False)

    tprs = np.zeros((nrof_folds,nrof_thresholds))
    fprs = np.zeros((nrof_folds,nrof_thresholds))
    accuracy = np.zeros((nrof_folds))

    num = embeddings1.shape[0]
    dist = np.zeros(num)
    for i in range(num):
        cos_dis_tmp = cosine_distance_numpy(embeddings1[i] , embeddings2[i])
        dist[i] = 1 - (cos_dis_tmp + 1.0)/2.0
